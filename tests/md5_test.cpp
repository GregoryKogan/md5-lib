...
TEST(BoundaryConditionTests, LessThanOneBlock_PaddingBoundary_55_Bytes) {
  std::string input(55, 'a');
  RunTest(input, "ef1772b6dff9a122358552954ad0df65");
}

TEST(BoundaryConditionTests, LessThanOneBlock_PaddingBoundary_56_Bytes) {
  std::string input(56, 'a');
  RunTest(input, "3b0c8ac703f828b04c6c197006d17218");
}

TEST(BoundaryConditionTests, LessThanOneBlock_PaddingBoundary_63_Bytes) {
  std::string input(63, 'a');
  RunTest(input, "b06521f39153d618550606be297466d5");
}

TEST(BoundaryConditionTests, ExactlyOneBlock_64_Bytes) {
  std::string input(64, 'a');
  RunTest(input, "014842d480b571495a4a0363793f7367");
}

TEST(BoundaryConditionTests, JustOverOneBlock_65_Bytes) {
  std::string input(65, 'a');
  RunTest(input, "c743a45e0d2e6a95cb859adae0248435");
}

TEST(APITest, ThrowsOnNullptr) {
  EXPECT_THROW(static_cast<void>(md5_lib::CalculateMD5(nullptr, 10)),
...
class LargeInputTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    for (std::size_t bytes_written = 0; bytes_written < kFileSize;
         bytes_written += buffer.size()) {
      const std::size_t bytes_to_write =
          std::min(buffer.size(), kFileSize - bytes_written);
...
TEST_F(LargeInputTest, StreamMatchesMemoryForLargeFile) {
  ...
  const std::string stream_hash = md5_lib::CalculateMD5(infile);
